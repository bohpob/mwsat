...
public:
    static void parseWeights(istringstream &iss, vector<shared_ptr<CLiteral> > &literals) {
        ...
        while (iss >> weight && weight != 0) {
            literals.emplace_back(make_shared<CLiteral>(i, weight));
            i++;
        }
    }

    static void parseClause(istringstream &iss, vector<shared_ptr<CLiteral> > &literals, vector<CClause> &clauses) {
        CClause clause;
        int literal;
    ...
    static bool readFromFile(const string &filename, vector<CClause> &clauses,
